...
_js="<div id='w' style='width:100%;height:75vh;overflow-y:auto;overflow-x:hidden;position:relative;background:#0d0d0d;'><canvas id='q'></canvas></div><script>const cv=document.getElementById('q'),cx=cv.getContext('2d'),wr=document.getElementById('w');let W=wr.offsetWidth;const cfg=V_JC,si={'s':{l:0,b:1},'p':{l:1,b:3},'d':{l:2,b:5},'f':{l:3,b:7}};function gG(c,x,y,w,h){let g=c.createLinearGradient(x,y,x+w,y);g.addColorStop(0,'#78ffd6');g.addColorStop(0.33,'#4a00e0');g.addColorStop(0.66,'#c70039');g.addColorStop(1,'#ffb6c1');return g;}function cH(){let sx=50,cx=sx,cy=100,bs=45,m=50;for(let[s,e]of Object.entries(cfg)){let t=s.slice(-1),nb=si[t].b,bw=nb*bs;if(cx+bw>W-50){cx=sx;cy+=130;}cx+=bw+m;}return cy+150;}function rR(){W=wr.offsetWidth;let nH=cH();cv.width=W;cv.height=Math.max(wr.offsetHeight,nH);r();}window.addEventListener('resize',rR);function dA(x,y,u){cx.beginPath();cx.strokeStyle='#FF007F';cx.lineWidth=2.5;cx.shadowBlur=8;cx.shadowColor='#FF007F';let l=22,sy=u?y+l/2:y-l/2,ey=u?y-l/2:y+l/2;cx.moveTo(x,sy);cx.lineTo(x,ey);let ho=u?6:-6;cx.moveTo(x-5,ey+ho);cx.lineTo(x,ey);cx.lineTo(x+5,ey+ho);cx.stroke();cx.shadowBlur=0;}function r(){cx.fillStyle='#0d0d0d';cx.fillRect(0,0,cv.width,cv.height);cx.font='bold 6vw Courier New';cx.fillStyle='rgba(255,255,255,0.02)';cx.textAlign='center';cx.textBaseline='middle';cx.fillText('@anim.VisualX',cv.width/2,cv.height/2);let sx=50,c_x=sx,cy=100,bs=45,m=50;cx.font='16px monospace';cx.textAlign='center';for(let[s,e]of Object.entries(cfg)){let t=s.slice(-1),i=si[t],nb=i.b,bw=nb*bs;if(c_x+bw>W-50){c_x=sx;cy+=130;}cx.fillStyle='#fff';cx.fillText(s+' ('+e+'e-)',c_x+bw/2,cy+bs+25);for(let j=0;j<nb;j++){let bx=c_x+j*bs,by=cy;cx.beginPath();cx.strokeStyle='#00FFFF';cx.lineWidth=2;cx.shadowBlur=10;cx.shadowColor='#00FFFF';cx.rect(bx,by,bs,bs);cx.stroke();let ml=j-i.l;cx.shadowBlur=0;cx.fillStyle='#ccc';cx.font='12px monospace';cx.fillText((ml>0?'+':'')+ml,bx+bs/2,by-10);let eC=0;if(e>j)eC++;if(e>j+nb)eC++;if(eC>=1)dA(bx+bs/2-6,by+bs/2,true);if(eC===2)dA(bx+bs/2+6,by+bs/2,false);}c_x+=bw+m;}cx.fillStyle=gG(cx,0,cy+100,cv.width,4);cx.fillRect(50,cy+90,cv.width-100,4);}setTimeout(rR,100);</script>"
