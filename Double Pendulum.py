...
_js = "<div id='wC' style='width:100%;height:90vh;overflow:hidden;position:relative;background:#050505;'><canvas id='pC'></canvas></div><script>const c=document.getElementById('pC'),x=c.getContext('2d');let w,h,r1=V_L1,r2=V_L2,m1=V_M1,m2=V_M2,g=V_G,a1=Math.PI/2.2,a2=Math.PI/2.5,v1=0,v2=0,p=[],mp=V_TR;function r(){w=c.width=window.innerWidth;h=c.height=window.innerHeight;}window.addEventListener('resize',r);r();function A(){for(let i=0;i<3;i++){let n1=-g*(2*m1+m2)*Math.sin(a1),n2=-m2*g*Math.sin(a1-2*a2),n3=-2*Math.sin(a1-a2)*m2,n4=v2*v2*r2+v1*v1*r1*Math.cos(a1-a2),d=r1*(2*m1+m2-m2*Math.cos(2*a1-2*a2)),A1=(n1+n2+n3*n4)/d;n1=2*Math.sin(a1-a2);n2=(v1*v1*r1*(m1+m2));n3=g*(m1+m2)*Math.cos(a1);n4=v2*v2*r2*m2*Math.cos(a1-a2);d=r2*(2*m1+m2-m2*Math.cos(2*a1-2*a2));let A2=(n1*(n2+n3+n4))/d;v1+=A1*0.2;v2+=A2*0.2;a1+=v1;a2+=v2;}let x1=r1*Math.sin(a1),y1=r1*Math.cos(a1),x2=x1+r2*Math.sin(a2),y2=y1+r2*Math.cos(a2);p.push({x:x2,y:y2});if(p.length>mp)p.shift();x.fillStyle='#050505';x.fillRect(0,0,w,h);const tl=r1+r2,pd=60,ah=h-pd*2,sc=Math.min(1.0,ah/(tl*2));x.save();x.translate(w/2,h/2);x.scale(sc,sc);x.beginPath();x.lineWidth=1.5/sc;x.strokeStyle='#00FFFF';for(let i=1;i<p.length;i++){x.moveTo(p[i-1].x,p[i-1].y);x.lineTo(p[i].x,p[i].y);}x.stroke();x.strokeStyle='#FFFFFF';x.lineWidth=3/sc;x.lineCap='round';x.beginPath();x.moveTo(0,0);x.lineTo(x1,y1);x.lineTo(x2,y2);x.stroke();x.beginPath();x.arc(0,0,8/sc,0,Math.PI*2);x.fillStyle='#444444';x.fill();x.beginPath();x.arc(0,0,4/sc,0,Math.PI*2);x.fillStyle='#FFFFFF';x.fill();x.fillStyle='#FF007F';x.beginPath();x.arc(x1,y1,8/sc,0,Math.PI*2);x.fill();x.beginPath();x.arc(x2,y2,10/sc,0,Math.PI*2);x.fill();x.restore();requestAnimationFrame(A);}A();</script><style>body{margin:0;overflow:hidden;}</style>"
